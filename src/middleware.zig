...
fn initCAPIResponseStorage() void {
    if (!c_api_response_storage_init) {
        c_api_response_storage = std.StringHashMap(Response).init(std.heap.page_allocator);
...
pub fn storeCAPIResponse(req: *Request, resp: Response) !void {
    ...
    const id_copy = try std.heap.page_allocator.dupe(u8, request_id);
    errdefer std.heap.page_allocator.free(id_copy);
